...
let slot_time_duration_morning;
...
$('#slot_time_duration_morning').on('change',function(){
    var selectedTime = $(this).val();
    if (selectedTime) {
        slot_time_duration_morning = selectedTime;
...
function calculate_time_slots_morning() {
    ...
    let timeperslot = convertTimeToMinutes(slot_time_duration_morning)

    if (endMinutes > startMinutes) {
        let time_of_the_morning_shift = endMinutes - startMinutes;
        let average_time = timeperslot; // 10 minutes per slot
...
$('#slot_time_evening').on('change',function(){
    var selectedTime = $(this).val();
    if (selectedTime) {
        slot_time_evening = selectedTime;
        if (slot_time_evening) {
...
function calculate_time_slots_evening() {
    ...
    let timeperslotevening = convertTimeToMinutes(slot_time_evening)

    if (endMinutes > startMinutes) {
        let time_of_the_evening_shift = endMinutes - startMinutes;
        let average_time = timeperslotevening; // 10 minutes per slot
